...
function splitAndCombine(arr) {
    // 자를 인덱스 구하기
    const cutIndex = arr.length / 2;

    // 배열 나누기
    let arr1 = arr.slice(0, cutIndex);
    let arr2 = arr.slice(cutIndex);
...
function getDiscountedMenus(menus) {
    ...
    let newMenus = menus.map((menu) => {
        let newMenu = { ...menu, discountedPrice: menu.price - 500 };
        return newMenu;
    });
...
function getSortedBooks(books) {
    let sortedBooks = books.toSorted(
